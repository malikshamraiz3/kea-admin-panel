...
const ConfirmModal: React.FC<ConfirmModalProps> = ({ itemName, onConfirm, onCancel, isDeleting }) => (
  ...
  >
    ...
    >
      ...
      <h3 style={{ fontSize: 20, fontWeight: 700, color: "#f1f5f9", margin: "0 0 10px" }}>Subject Delete Karein?</h3>
...
const SubjectsPage: React.FC = () => {
  ...
  const className = query.get("className") || "Selected Class";
  ...
  useEffect(() => {
    ...
    const loadSubjects = async () => {
      ...
      try {
        ...
        if (isActive) setSubjects(data ?? []);
      } catch {
        if (isActive) setToast({ message: "Subjects load nahi hui. Backend check karein.", type: "error" });
      } finally {
  ...
  const handleAddSubject = async (e: React.FormEvent) => {
    ...
    if (!trimmed) {
      setAddError("Subject ka naam likhna zaroori hai.");
      return;
    }
    if (!classId) {
      setAddError("Pehle class select karein.");
      return;
    }
    if (subjects.some((s) => s.name.toLowerCase() === trimmed.toLowerCase())) {
      setAddError("Yeh subject pehle se exist karti hai.");
      return;
    }
    ...
    setAddError("");
    try {
      const created = await createSubject(classId, trimmed);
      setSubjects((prev) => [created, ...prev]);
      setNewSubjectName("");
      setShowAddForm(false);
      showToast(`"${created.name}" successfully add ho gayi! 🎉`, "success");
    } catch {
      showToast("Subject add nahi ho saki. Dobara try karein.", "error");
    } finally {
      setIsAdding(false);
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setSubjects((prev) => prev.filter((subject) => subject._id !== deleteTarget._id));
  ...
  const filtered = useMemo(() => subjects.filter((subject) => subject.name.toLowerCase().includes(searchQuery.toLowerCase())), [subjects, searchQuery]);
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <header style={{ height: 64, background: "rgba(12,21,38,0.9)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(148,163,184,0.08)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", position: "sticky", top: 0, zIndex: 40 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#64748b" }}>
              ...
              <ChevronRightIcon />
              <span style={{ color: "#60a5fa", fontWeight: 600 }}>Subjects</span>
            </div>
          </div>
        ...
        <div style={{ flex: 1, padding: "28px", overflowY: "auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28, flexWrap: "wrap", gap: 16 }}>
            <div>
              <h1 style={{ fontSize: 26, fontWeight: 800, color: "#f1f5f9", margin: "0 0 6px", letterSpacing: "-0.5px" }}>📘 Subjects Management</h1>
              <p style={{ fontSize: 14, color: "#94a3b8", margin: 0 }}>Subjects add, dekhen aur delete karein</p>
            </div>
            <button
            ...
            >
              <PlusIcon />
              New Subject Add Karein
            </button>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 14, marginBottom: 24 }}>
            {[
              { label: "Selected Class", value: className, icon: "🎓", color: "#3b82f6" },
              { label: "Total Subjects", value: subjects.length, icon: "📘", color: "#06b6d4" },
              ...
              <div key={index} style={{ background: "rgba(30,41,59,0.7)", border: "1px solid rgba(148,163,184,0.1)", borderRadius: 12, padding: "14px 18px", display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div>
                  <div style={{ fontSize: item.label === "Selected Class" ? 13 : 22, fontWeight: 700, color: "#f1f5f9", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 100 }}>{item.value}</div>
          ...
          {showAddForm && (
            <div style={{ background: "rgba(30,41,59,0.85)", border: "1px solid rgba(59,130,246,0.25)", borderRadius: 16, padding: "24px", marginBottom: 24 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
                <h3 style={{ fontSize: 16, fontWeight: 700, color: "#f1f5f9", margin: 0 }}>➕ Nayi Subject Add Karein</h3>
                <button onClick={() => { setShowAddForm(false); setAddError(""); setNewSubjectName(""); }} style={{ background: "none", border: "none", color: "#64748b", cursor: "pointer", display: "flex", padding: 4, borderRadius: 6 }}>
                  <XIcon />
                </button>
              </div>
              <form onSubmit={handleAddSubject} style={{ display: "flex", gap: 12, alignItems: "flex-start", flexWrap: "wrap" }}>
                <div style={{ flex: 1, minWidth: 200 }}>
                  <input
                    type="text"
                    value={newSubjectName}
                    onChange={(e) => { setNewSubjectName(e.target.value); setAddError(""); }}
                    placeholder="Maslan: English, Math, Physics..."
                    autoFocus
                ...
                <button type="submit" disabled={isAdding} style={{ padding: "12px 22px", background: isAdding ? "rgba(59,130,246,0.5)" : "linear-gradient(135deg,#1e40af,#3b82f6)", border: "none", borderRadius: 10, color: "#fff", fontSize: 14, fontWeight: 600, cursor: isAdding ? "not-allowed" : "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 8 }}>
                  {isAdding ? "Adding..." : <><PlusIcon /> Add Subject</>}
                </button>
              </form>
            </div>
          )}
          ...
          {isLoading ? (
            ...
            <div style={{ textAlign: "center", padding: "60px 20px" }}>
              ...
              <h3 style={{ fontSize: 18, fontWeight: 700, color: "#f1f5f9", margin: "0 0 8px" }}>Pehle class select karein</h3>
              <p style={{ fontSize: 14, color: "#64748b", margin: "0 0 24px" }}>Classes page se kisi class ka subjects dekhain.</p>
              <button onClick={() => navigate("/classes")} style={{ padding: "10px 22px", background: "linear-gradient(135deg,#1e40af,#3b82f6)", border: "none", borderRadius: 10, color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
                Go to Classes
              </button>
            </div>
          ) : filtered.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 20px" }}>
              <div style={{ fontSize: 56, marginBottom: 16 }}>📭</div>
              <h3 style={{ fontSize: 18, fontWeight: 700, color: "#f1f5f9", margin: "0 0 8px" }}>{searchQuery ? `"${searchQuery}" nahi mili` : "Koi subject nahi hai"}</h3>
              <p style={{ fontSize: 14, color: "#64748b", margin: "0 0 24px" }}>{searchQuery ? "Search query badlen ya nayi subject add karein." : "Upar wala button click karke pehli subject add karein."}</p>
              {!searchQuery && (
            ...
            <div style={{ display: "grid", gridTemplateColumns: viewMode === "grid" ? "repeat(auto-fill,minmax(260px,1fr))" : "1fr", gap: viewMode === "grid" ? 16 : 10 }}>
              {filtered.map((subject) => (
                <div key={subject._id} style={{ background: "rgba(30,41,59,0.7)", border: "1px solid rgba(148,163,184,0.1)", borderRadius: viewMode === "grid" ? 16 : 12, padding: viewMode === "grid" ? "22px 20px" : "14px 20px", transition: "all 0.25s ease", position: "relative", overflow: "hidden", display: viewMode === "list" ? "flex" : "block", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }} onClick={() => navigate(`/documents?subjectId=${subject._id}&subjectName=${encodeURIComponent(subject.name)}`)} onMouseEnter={(e) => e.currentTarget.style.borderColor = "rgba(59,130,246,0.4)"} onMouseLeave={(e) => e.currentTarget.style.borderColor = "rgba(148,163,184,0.1)"}>
                  ...
                  <div style={{ display: "flex", alignItems: viewMode === "grid" ? "flex-start" : "center", gap: 14, flex: 1 }}>
                    ...
                    {viewMode === "grid" ? (
                      <div style={{ flex: 1 }}>
                        <h3 style={{ fontSize: 16, fontWeight: 700, color: "#f1f5f9", margin: "0 0 5px", textTransform: "capitalize", letterSpacing: "-0.2px" }}>{subject.name}</h3>
                        <p style={{ fontSize: 12, color: "#64748b", margin: "0 0 16px" }}>Added: {formatDate(subject.createdAt)}</p>
                        <button onClick={(e) => { e.stopPropagation(); setDeleteTarget(subject); }} style={{ padding: "8px 12px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 8, color: "#f87171", fontSize: 12, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 5, fontWeight: 600 }}>
                          <TrashIcon /> Delete
                        </button>
                      </div>
                    ) : (
                      <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 20 }}>
                        <div>
                          <h3 style={{ fontSize: 15, fontWeight: 700, color: "#f1f5f9", margin: 0, textTransform: "capitalize" }}>{subject.name}</h3>
                          <p style={{ fontSize: 12, color: "#64748b", margin: "2px 0 0" }}>Added: {formatDate(subject.createdAt)}</p>
                        </div>
                      </div>
                    )}
                  </div>
                  {viewMode === "list" && (
                    <div style={{ display: "flex", gap: 8, flexShrink: 0, marginLeft: 16 }}>
                      <button onClick={(e) => { e.stopPropagation(); setDeleteTarget(subject); }} style={{ padding: "7px 12px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 8, color: "#f87171", fontSize: 12.5, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 5, fontWeight: 600 }}>
                        <TrashIcon /> Delete
                      </button>
                    </div>
                  )}
